...
;
;
const LogoBrandDownload = (param)=>{
    let { children, files, className } = param;
    const handleDownload = async (file)=>{
        try {
            const response = await fetch(file.path);
            if (!response.ok) throw new Error("Failed to fetch ".concat(file.name));
...
const Logo = (param)=>{
    let { url, className, children, ...props } = param;
...
//# sourceMappingURL=components_e43e8132._.js.map